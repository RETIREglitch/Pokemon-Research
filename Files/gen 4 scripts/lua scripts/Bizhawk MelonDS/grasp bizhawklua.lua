...
function text(x, y, text, color,screen)
	screen = screen or 1
	gui.text(x, screen_y[screen]+y,text,color,20)
end
...
function draw_bounding_rectangle(x,y,width,height,fill,border_clr,screen)
	screen = screen or 1
	gui.drawBox(x-(width/2),screen_y[screen]+y-(height/2),x+(width/2),screen_y[screen]+y+(height/2),fill,border_clr)
end 

function draw_rectangle(x,y,width,height,fill,border_clr,screen)
	screen = screen or 1
	gui.drawBox(x,screen_y[screen]/2+y,x+width,screen_y[screen]/2+y+height,fill,border_clr)
end 

function draw_line(x,y,width,height,clr,screen)
	screen = screen or 1
	gui.drawLine(x,screen_y[screen]+y,x+width,screen_y[screen]+y+height,clr)
end 
...
screen_y = screen_options["OW"]

function set_screen_params(memory_state)
	if screen_options[memory_state] ~= nil then
		return screen_options[memory_state]
	end 
	return screen_options["OW"]
end
...
function find_item_address_from_pocket(item_id,pocket_id,item_id2)
	...
	current_item_id = memory.read_u16_le(current_addr)
...
function switch_to_item(item_id)
	...
	current_pocket_id = memory.read_u8(base + data_table["current_pocket_index_offs"])
	...
	wait_frames(40)
	current_pocket_id = memory.read_u8(base + data_table["current_pocket_index_offs"])	
	hovering_item_id_offs = memory.read_u32_le(data_table["bag_hovering_data_ptr"])

	if memory.read_u16_le(hovering_item_id_offs + hovering_item_struct["cursor_offset_16"]) == 0x5A then -- hovering over "none"
		direction = "Up"
	else	
		hovering_item_id = memory.read_u8(hovering_item_id_offs + hovering_item_struct["hovering_item_id"])
	...
	while item_id%0x100 ~= hovering_item_id do
		press_button(direction)
		wait_frames(2)
		hovering_item_id = memory.read_u8(hovering_item_id_offs + 0x360)
...
function set_stepcounter(steps)
	...
	memory.write_u16_le(step_addr,steps)
end 

function add_to_stepcounter(steps)
	step_addr = base+live_struct["step_counter"]
	memory.write_u32_le(step_addr,memory.read_u32_le(step_addr)+steps)
end 
...
function check_battle_state()
	if memory.read_u16_le(data_table["battle_check"]) ~= 0 then
		return true
	end
	return false 
end 
...
function show_player_data()
	x_phys_32 = memory.read_u16_le(base + player_struct["x_phys_32"] + memory_shift)
	z_phys_32 = memory.read_u16_le(base + player_struct["z_phys_32"] + memory_shift) 
	map_id_phys_32 =  memory.read_u16_le(base + live_struct["map_id_32"])
	text(10,30,"Physical:","yellow")
	text(40,45,"X: "..x_phys_32..","..fmt(x_phys_32,4),"yellow")
	text(40,60,"Z: "..z_phys_32..","..fmt(z_phys_32,4),"yellow")
	text(40,75,"Map Id: "..map_id_phys_32..","..fmt(map_id_phys_32,4),get_edit_color(map_editing))
	x_stored_warp_16 = memory.read_u16_le(base + live_struct["x_stored_warp_16"])
	z_stored_warp_16 = memory.read_u16_le(base + live_struct["z_stored_warp_16"])	
	text(10,95,"Stored Warp:","yellow")
	text(40,110,"X: "..x_stored_warp_16..","..fmt(x_stored_warp_16,4),"yellow")
	text(40,125,"Z: "..z_stored_warp_16..","..fmt(z_stored_warp_16,4),"yellow")
	step_counter = memory.read_u16_le(base + live_struct["step_counter"])
	text(10,140,"Steps: "..step_counter,"yellow")
	npc_count = memory.read_u8(base + general_npc_struct["npc_count"] + memory_shift) -1 
	text(10,155,"NPCs: "..npc_count,"yellow")
	text(10,170,"TP: "..tp_amount,get_edit_color(teleport_editing))
	
end

function win_ug_minigame()
	for i = 0,data_table["ug_excavation_minigame_tile_count"] do
		memory.write_u8(base+ug_excavation_minigame_struct["tiles_start"]+i,0)		
	end 
end
...
function show_ug_gems() 
	draw_bounding_boxes(data_table["ug_gem_count"],base + ug_gem_struct["x_phys_16"], base + ug_gem_struct["z_phys_16"],data_table["ug_gem_struct_size"],0x0,0xD0ffff66,0x60ffff66)
end 

function show_ug_traps()
	ug_trap_count = memory.read_u8(data_table["ug_trap_count_addr"])
	draw_bounding_boxes(ug_trap_count,base + ug_trap_struct["x_phys_16"], base + ug_trap_struct["z_phys_16"],data_table["ug_trap_struct_size"],0x0,0xD0ffffff,0x60ffffff)
end 

function show_objects()
	object_count = memory.read_u16_le(base+object_struct["object_count"])
	draw_bounding_boxes(object_count,base + object_struct["x_phys_32"], base + object_struct["z_phys_32"],data_table["object_struct_size"],0x0,0xD09793FF,0x609793FF)
end 

function show_triggers()
	start_trigger_struct = memory.read_u32_le(base+object_struct["trigger_ptr_offs"])
	trigger_count = memory.read_u16_le(start_trigger_struct+trigger_struct["trigger_count"])
	for i = 0,trigger_count-1 do 
		x_phys_16 = memory.read_u16_le(start_trigger_struct + trigger_struct["x_phys_16"] + i*data_table["trigger_struct_size"])
		z_phys_16 = memory.read_u16_le(start_trigger_struct + trigger_struct["z_phys_16"] + i*data_table["trigger_struct_size"])
		vert_count_16 = memory.read_u16_le(start_trigger_struct+trigger_struct["vert_count_16"] + i*data_table["trigger_struct_size"])
		hor_count_16 = memory.read_u16_le(start_trigger_struct+trigger_struct["hor_count_16"] + i*data_table["trigger_struct_size"])
		for vert_offs = 0, vert_count_16-1 do
			...
			for hor_offs = 0, hor_count_16-1 do
				...
				draw_bounding_box(x_trigger_inst,z_trigger_inst,0xD0fff866,0x60fff866)		
			end 
		end 
	end
end
...
function show_warps()
	start_warp_struct = memory.read_u32_le(base+object_struct["warp_ptr_offs"])
	warp_count = memory.read_u16_le(start_warp_struct+warp_struct["warp_count"])
	draw_bounding_boxes(warp_count,start_warp_struct+warp_struct["x_phys_16"],start_warp_struct+warp_struct["z_phys_16"],data_table["warp_struct_size"],0x0,0xD0600038, 0x60600038)
end 

function show_npcs()
	npc_count = memory.read_u8(base + general_npc_struct["npc_count"] + memory_shift) -1 -- subtracting player's npc from count
	if npc_count > 0 then -- prevent negative npc count when not initialized
		draw_bounding_boxes(npc_count,base + generic_npc_struct["x_phys_32"],base + generic_npc_struct["z_phys_32"],data_table["npc_struct_size"],memory_shift,0xD088ffff,0x6088ffff)
	end
end
...
function draw_bounding_boxes(count,x_start,z_start,struct_size,extra_offs,fill,border)
	for i = 0,count-1 do 
		x_phys_32 = memory.read_u16_le(x_start + i*struct_size + extra_offs)
		z_phys_32 = memory.read_u16_le(z_start + i*struct_size + extra_offs)
		draw_bounding_box(x_phys_32,z_phys_32,fill,border)
	end
end 

function draw_bounding_box(x,z,fill_clr,border_clr)
	x_v = (memory.read_u16_le(base + live_struct["x_pos_32_r"]) - x) *16
	y_v = (memory.read_u16_le(base + live_struct["z_pos_32_r"]) - z)
	if y_v > 0 then 
		y_v = y_v*13
	else
		y_v = y_v*15.5
...
function show_bounding_boxes(memory_state)
	...
	if memory_state == "UG" then
		-- if memory.read_u8(base+data_table["menu_addr"]) == 0 then 
		show_ug_gems()
		show_ug_traps()
		-- 	return 
		-- end
...
function show_void_pos()
	matrix_width = memory.read_u8(base + matrix_struct["matrix_width_8"])
	...
	for row=0,9 do
		for col=0,18 do 
			...
			c_map_id = math.min(memory.read_u16_le(c_map_offset),1000)

			
			if (c_map_offset == matrix_center + x_offs+z_offs) and (row==5) then
				-- text(90,10,fmt(c_map_offset,8).."\n"..data_table["matrix_size"])
				clr = 'white'
			else
				clr = get_map_id_color(c_map_id)
			end
			text(3 + row*50,3 + col*20,get_display_map(c_map_id),clr,2)
		end 
	end 
end
...
function show_chunks_ow()
	...
	draw_rectangle(0,0,256,200,0xff0001ff,0xff0001ff,2)
...
function show_tile_data()
	tile_id = memory.read_u8(base + player_struct["tile_type_16_1"] + memory_shift)
	-- current_tile_2 = memory.read_u8(base + player_struct["tile_type_16_2"] + memory_shift) -- slower
	text(143,15,"Tile: "..tile_id..","..fmt(tile_id,2),'yellow')
	text(143,25,tile_names[tile_id+1],'yellow')

	start_chunk_struct = memory.read_u32_le(base+data_table["chunk_calculation_ptr"])
	text(143,35,"Loading:",'yellow')
	load_state = memory.read_u8(start_chunk_struct + chunk_struct["load_state"])
	if load_state == 0 then
		text(198,35,"true",'green')
		
	else 
		text(198,35,"false",'red')
	end
	
	if show_load_calculations then 
		x_target_16 = memory.read_u16_le(start_chunk_struct + chunk_struct["x_target_16"])
		z_target_16 = memory.read_u16_le(start_chunk_struct + chunk_struct["z_target_16"])
		text(143,45,"Target:",'yellow')
		text(143,55,"    X: "..x_target_16..","..fmt(x_target_16,4),'yellow')
		text(143,65,"    Z: "..z_target_16..","..fmt(z_target_16,4),'yellow')

		x_cam_16 = memory.read_u16_le(base + player_struct["x_cam_16"] + memory_shift)
		z_cam_16 = memory.read_u16_le(base + player_struct["z_cam_16"] + memory_shift) 

		text(143,75,"Graphical:",'yellow')
		text(143,85,"    X: "..x_cam_16..","..fmt(x_cam_16,4),'yellow')
		text(143,95,"    Z: "..z_cam_16..","..fmt(z_cam_16,4),'yellow')
	end 
...
function show_tiles_ow(additional_offset)
	...
	for i = 1,#chunk_pointer_offs do
		chunk_pointer = memory.read_u32_le(chunk_pointer_offs[i] + start_chunk_struct)
		for col = 0,31 do 

			for row = 0,31 do 
				tile_data = memory.read_u16_le(chunk_pointer+row*2 + col*64+additional_offset)
...
function show_collision_ow(additional_offset)
	start_chunk_struct = memory.read_u32_le(base+data_table["chunk_calculation_ptr"])
	--text(153,140,"0x"..fmt(start_chunk_struct,8))
	chunk_pointer_offs = chunk_struct["chunk_pointer_offs"]
	chunk_pointers = {}
	--text(153,150,"Chunk addresses:")
	for i = 1,#chunk_pointer_offs do
		chunk_pointer = memory.read_u32_le(chunk_pointer_offs[i] + start_chunk_struct)
		--text(153,150+i*10,"0x"..fmt(chunk_pointer,7))
		for row = 0,31 do 
			for col = 0,31 do 
				if (memory.read_u8(chunk_pointer+row*2 + col*64+additional_offset)) ~= 0xff then
					collision  = memory.read_u8(chunk_pointer+row*2 + col*64+1+additional_offset)
...
function show_chunk_position()
	start_chunk_struct = memory.read_u32_le(base+data_table["chunk_calculation_ptr"])
	c_chunk = memory.read_u8(chunk_struct["current_chunk_8"] + start_chunk_struct)
	if c_chunk < 4 then 
		x_phys_32 = memory.read_u32_le(base + player_struct["x_phys_32"] + memory_shift)
		z_phys_32 = memory.read_u32_le(base + player_struct["z_phys_32"] + memory_shift)
		if x_phys_32 > 0x7FFFFFF then 
...
function show_script_start()
	...
	script_map = memory.read_u16_le(base + live_struct["map_id_32"])

	script_array_pointer = base + 0x29574 + memory_shift-- this only accounts for normal RETIRE 
	script_array_addr = memory.read_u32_le(script_array_pointer)

	temp_script_offs_4_addr = script_array_addr + 4*3 -- 4th index, 4 bytes each (indexing from 0 instead of 1)
	temp_script_execution_start_addr = temp_script_offs_4_addr + memory.read_u32_le(temp_script_offs_4_addr) + 0x4-- add the offset to its own address, add 4 bcs jump starts after the address

	if script_array_addr == 0 then 
		script_executed = false
		text(143,15,"No script loaded","#00f0f")
		return 
	end 
	...
	if (script_executed and update_variables) then
		...
		script_offs_4 = memory.read_u32_le(script_offs_4_addr)
	...
	if show_script then 
		text(143,15,"Next Command:\n    0x"..fmt(next_opcode_addr,8),"#00f0f")
		text(143,35,"Script Data:","#00f0f")
		text(153,45,"Array:  0x"..fmt(script_array_addr,0),"#00f0f")
		text(153,55,"Offset: 0x"..fmt(script_offs_4,0),"#00f0f")
		text(153,65,"Exec:   0x"..fmt(script_execution_start_addr,0),"#00f0f")
		show_script_memory()
	else
		text(143,15,"No script loaded","#00f0f")
	end 
...
function get_internal_pointer()
	...
	next_opcode_addr = memory.read_u32_le(opcode_pointer)
	-- opcode = memory.read_u16_le(next_opcode_addr-2)
	-- if next_opcode_addr%2 == 1 then 
	-- 	higher_byte = bit.band(memory.read_u16_le(next_opcode_addr),0xFF)
	-- 	lower_byte = bit.rshift(memory.read_u16_le(next_opcode_addr-2),8)
	-- 	opcode = bit.bor(bit.lshift(higher_byte,8),lower_byte)
	-- end 
	--text(143,185,"Pointer addr:\n    "..fmt(opcode_pointer,8),"yellow")
end
...
function check_jump_validity(scr_cmd,par)
	...
	if (scr_cmd == 0x19) then 
		player_facing_direction = memory.read_u8(base + player_struct["facing_dir_32"] + memory_shift)
...
function show_script_memory()
	...
	cur_sc_id = bit.bor(shiftl(memory.read_u8(cur_sc_addr+1),8),memory.read_u8(cur_sc_addr))
	...
	while cur_sc do
		...
		cur_sc_id = bit.bor(shiftl(memory.read_u8(cur_sc_addr+1),8),memory.read_u8(cur_sc_addr))
		text(46+(index%8)*26,2+math.floor(index/8)*10,fmt(cur_sc_id,4),get_script_command_color(cur_sc_id),2)
		cur_sc = script_commands[cur_sc_id]

		if cur_sc then 
			if index%8 == 0 then 
				text(1,2+math.floor(index/8)*10,fmt(cur_sc_addr,7),addr_color,2)
			end 
			if halting_commands[cur_sc_id] then 
				index = (index - index%8) + 8
				text(46+(index%8)*26,2+math.floor(index/8)*10,"EXECUTION HALTED: "..cur_sc['script_command'],"red",2)
				break
			end 
			...
			if param_count > 0 then
				...
				if param_count%2 == 0 then
					for i = 0, param_count-1,2 do 
						...
						cur_param_id = bit.bor(shiftl(memory.read_u8(cur_param_addr+1),8),memory.read_u8(cur_param_addr))
						jump_value = bit.bor(jump_value,shiftl(cur_param_id,8*i))
						index = index + 1
						text(46+(index%8)*26,2+math.floor(index/8)*10,fmt(cur_param_id,4),param_color,2)
						if index%8 == 0 then 
							text(1,2+math.floor(index/8)*10,fmt(cur_sc_addr,7),addr_color,2)
						end 
					end 
					...
					for i = 0, param_count-2,2 do 
						...
						if i ~= 0 then 
							...
							jump_value = memory.read_u8(cur_param_addr+1)
							jump_id = memory.read_u8(cur_param_addr)
						end 
						index = index + 1
						text(46+(index%8)*26,2+math.floor(index/8)*10,fmt(cur_param_id,4),param_color,2)
						if index%8 == 0 then 
					...
					cur_param_addr = cur_param_addr + 2
					cur_param_id = memory.read_u8(cur_param_addr)
					jump_value = bit.bor(jump_value,shiftl(cur_param_id,8*(param_count+2)))
					index = index + 1
					text(46+(index%8)*26,2+math.floor(index/8)*10,fmt(cur_param_id,2),param_color,2)
			...
			index = (index - index%8) + 8
			text(46+(index%8)*26,2+math.floor(index/8)*10,"EXECUTION HALTED","red",2)
			break
		end

		if check_jump_validity(cur_sc_id,jump_id) then
			text(143,85+10*jumps_shown,"Jump"..(jumps_shown+1)..": 0x"..fmt(jump_value,0),"yellow")
			index = (index - index%8) + 8
			cur_sc_addr = cur_sc_addr + jump_value
			text(46+(index%8)*26,2+math.floor(index/8)*10,"Jump by 0x"..fmt(jump_value,0).." to 0x"..fmt(cur_sc_addr+2,8),"red",2)
			index = index + 7
	...
	if index == 0 then
		text(2,2,"First encountered script command invalid.","red",2)
	end 
end 
...
function memory_viewer_8()
	...
	for y = 0,15 do
		for x = 0,7 do
			-- script_command = memory.read_u16_le(script_execution_start_addr+x*2+y*16)
			current_addr = memview_addr+x*2+y*16 + scroll
			value = bit.bor(shiftl(memory.read_u8(current_addr),8),memory.read_u8(current_addr+1))
			color = get_value_color(value)
			text(48+x*26,20+y*10,fmt(value,4),color,2)
		end
		text(2,20+y*10,fmt(memview_addr+y*16 + scroll,7),"#888888",2)
	end  
	for x = 0,15 do
		text(48+x*13,8,fmt(x,0),"#888888",2)
...
function memory_viewer_16()
	...
	for y = 0,15 do
		for x = 0,7 do
			-- script_command = memory.read_u16_le(script_execution_start_addr+x*2+y*16)
			current_addr = memview_addr+x*2+y*16 + scroll
			value = memory.read_u16_le(current_addr)
			color = get_value_color(value)
			text(48+x*26,20+y*10,fmt(value,4),color,2)
		end
		text(2,20+y*10,fmt(memview_addr+y*16 + scroll,7),"#888888",2)
...
function write_image_to_calculator()
	...
	for i = 0,23 do 
		for j = 0,23 do 
		memory.write_u16_le(start_a+block_length*i+j*2+(0xF*3*i),0xA1)
		end 
	end 
...
function auto_sprite_inc()
	sprite = memory.read_u16_le(base + player_struct["sprite_id_32"])
	memory.write_u16_le(base + player_struct["sprite_id_32"],sprite+1)
	use_menu(5)
	wait_frames(60)
	press_button("B",4)
end 
...
function display_options()
	...
	for i = 0,#function_options-1 do
		text(option_x + 5,option_y + 18 + i*10,function_options[i+1][2])
	end 
end 
...
function main()
	...
	--base = memory.read_u32_le(memory.read_u32_le(0x2002848)-4)
	...
	text(5,15,"Base:"..fmt(base,8),'yellow')
	show_player_data()
